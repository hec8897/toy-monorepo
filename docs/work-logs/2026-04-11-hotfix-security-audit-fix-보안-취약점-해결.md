...
- 각 패키지를 특정 안전 버전으로 핀닝: `vite@8.0.8`, `axios@>=1.15.0`, `minimatch@>=9.0.7`, `picomatch@4.0.4`, `lodash@4.18.1`
- 해결 후 `file-type` moderate 취약점 2개가 남아 있으나, 수정 버전(`>=21.3.2`)이 `@swc/cli` 의존성 체인에서 호환 불가 → high 이상만 목표였으므로 범위 외
