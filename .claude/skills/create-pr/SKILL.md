...
1. **새로 추가된 파일 분석**: 새 모듈, 컴포넌트, API 엔드포인트 확인
2. **주요 함수/클래스 변경**: 새로 추가되거나 수정된 핵심 로직 파악
3. **의존성 변경**: package.json 변경사항 확인
4. **설정 변경**: 환경변수, 설정 파일 변경 확인

**변경사항 요약 작성 기준:**

- 사용자 관점에서 어떤 기능이 추가/변경되었는지 설명
- 기술적 구현 세부사항보다 **무엇이 가능해졌는지** 강조
- BE/FE/공통으로 구분하여 주요 변경사항 나열
...
[이 PR이 해결하는 문제 또는 추가하는 기능을 1-3줄로 요약]

### BE (백엔드 변경사항이 있을 경우)

- 새로운 API 엔드포인트: `POST /api/xxx`
- 인증/인가 로직 변경
- DB 스키마 변경

### FE (프론트엔드 변경사항이 있을 경우)
